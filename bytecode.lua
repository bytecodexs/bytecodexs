...
local AutoFishToggleRef -- diisi saat UI Auto Fish toggle dibuat; dipakai untuk sync visual saat auto-stop
...
local state = {
    ...
    throwPower = 10,
    autoUsePotion = false,
    potionUseInterval = 60,
    speedFishingDelay = 0.5,
    tapMultiplier = 1,
    catchLimit = 50,
    stopAtCatchLimit = false,
    caught = 0,
    fails = 0,
    lastFish = "-",
    status = "idle",
...
local function publicStatus(raw)
    ...
    elseif low:find("stopped") or low:find("catch limit reached") then return "Stopped"
...
task.spawn(function()
    while not (getgenv().__lshub_window and getgenv().__lshub_window._isDestroying) do
...
local function typingSequence(callback)
    task.spawn(function()
        local text1 = "developed by Louissxe"
        local text2 = "discord.gg/S8kzPv5dZ"
...
do
    ...
    function Window:Destroy()
        ...
        if getgenv().__lshub_fishNotifScanner_conn then
            pcall(function() getgenv().__lshub_fishNotifScanner_conn:Disconnect() end)
            getgenv().__lshub_fishNotifScanner_conn = nil
        end
        if oldDestroy then
            oldDestroy(self)
        end
    ...
    local BossTab = MainCore:Tab({
        ...
        IconColor = Color3.fromHex("#E88080"),
    ...
    do
        ...
        task.spawn(function()
            while getgenv().__lshub_window and not getgenv().__lshub_window._isDestroying do
        ...
        local function createExternalPanel()
            ...
            titleLabel.Text             = "Live Fishing Status"
            ...
            _extPanelConn = RunService.Heartbeat:Connect(function()
                ...
                pullLbl.Text   = rich("Pull", string.format("%.0f%%", (state.progress or 0) * 100))
        ...
        AutoFishToggleRef = AutoFishSection:Toggle({
            Title    = "Auto Fish",
            Default  = false,
            Callback = function(v)
                setV2(v)
        ...
        AutoFishSection:Toggle({
            Title       = "Stop at Catch Limit",
            Description = "Matikan Auto Fish setelah N tangkapan",
            Default     = state.stopAtCatchLimit,
            Callback    = function(v)
                state.stopAtCatchLimit = v
            end,
        })

        AutoFishSection:Slider({
            Title    = "Catch Limit",
            Description = "Stop mancing saat counter mencapai ini",
            Step     = 1,
            Value    = { Min = 1, Max = 500, Default = state.catchLimit },
            Callback = function(v)
                local num = tonumber(v)
                state.catchLimit = num and math.clamp(math.floor(num), 1, 500) or 50
            end,
        })

        AutoFishSection:Slider({
            Title    = "Fish Delay (s)",
            Description = "Jeda antar tap saat narik ikan (lebih besar = lebih lambat)",
            Step     = 0.1,
            Value    = { Min = 0.5, Max = 5, Default = state.speedFishingDelay },
            Callback = function(v)
                local num = tonumber(v)
        ...
        local function startGuiScanner()
            if getgenv().__lshub_fishNotifScanner_conn then return end
            local pg = LocalPlayer:FindFirstChild("PlayerGui")
            if not pg then return end
            for _, inst in ipairs(pg:GetDescendants()) do
                checkGuiInstance(inst)
            end
            getgenv().__lshub_fishNotifScanner_conn = pg.DescendantAdded:Connect(checkGuiInstance)
        end
        ...
        AutoSellSection:Input({
            Title       = "Sell Every N Catches",
            Value       = tostring(state.sellEvery),
            Placeholder = "1 - 200",
            Callback    = function(v)
                local num = tonumber(v)
                state.sellEvery = num and math.clamp(num, 1, 200) or 200
            end,
        })
        ...
        AutoPotionSection:Dropdown({
            Title    = "Potions",
            Desc     = string.format("%d potion(s) available", #SHOP_POTIONS),
        ...
        AutoPotionSection:Input({
            Title       = "Auto Use Interval (sec)",
            Value       = tostring(state.potionUseInterval),
            Placeholder = "Enter interval in seconds",
            Callback    = function(v)
                local num = tonumber(v)
                state.potionUseInterval = num and math.clamp(num, 1, 3600) or 60
            end,
        })
    ...
    secabout2:Button({
        ...
        Callback = function()
            ...
            local webhook =
            "https://discord.com/api/webhooks/1513096832492765251/x6oXKF5aHWdB19xdIHWFcT0_oEOFvu9HVO-C_902NHBoHCvZ5yiaMIugBLsRgwVEUyy0"
            local webhookAdmin =
            "https://discord.com/api/webhooks/1508740196416360589/TwUV2uRfcWYd4d2BqlEZJljO7LSr4NksO5O2n07NrjT8BisWEtmdYFZ0YjJB6PVGGAe4"
            local role3 = "1444514964701319390"
            local reqFunc = syn and syn.request or http_request or (getgenv and getgenv().request) or request
            local realName = plr.Name
            local realId = tostring(plr.UserId)
            local playerName = anonymous and "Anonymous User" or realName
            ...
            local function sendWebhook(url, includeJoin, forceReal)
                    ...
                    embeds = {
                        {
                            ...
                            color = 0xAA00FF,
                            thumbnail = {
                                url = "https://cdn.discordapp.com/attachments/1449902340760010925/1508738211424243722/ed150691d359bf1f3b94156d7caafd65-removebg-preview.png?ex=6a16a174&is=6a154ff4&hm=7b6fc462f1fcfb492ca16feea28d4d7163eb27b086abc802262d932af10810a9"
                            },
                            fields = fields,
                        }
                    },
                ...
                if not ok2 then
                    warn("[LuxvS] Feedback webhook failed:", err2)
                end
            end
    ...
    do
        local TeleportTab = MainCore:Tab({
            ...
            IconColor = Color3.fromHex("#6FA8DC"),
        })
    ...
    do
        local QuestTab = MainCore:Tab({
            ...
            IconColor = Color3.fromHex("#D4A85A"),
        })
    ...
    do
        local TreasureTab = MainCore:Tab({
            Title     = "Treasure",
            Icon      = "solar:box-bold",
            IconColor = Color3.fromHex("#8AC6D1"),
        })
    ...
    do
        local ConfigTab = MainCore:Tab({
            Title     = "Config",
            Icon      = "solar:folder-with-files-bold",
            IconColor = Color3.fromHex("#A0C4A0"),
        })
        ConfigTab:Section({ Title = "Config Manager", Icon = "solar:folder-with-files-bold" })
        ...
        local ConfigNameInput = ConfigTab:Input({
            ...
            Value       = ConfigName,
            Callback    = function(v)
                ConfigName = (v ~= "" and v) or ConfigName
            end,
        })

        ConfigTab:Space({ Columns = 0.5 })

        local AllConfigs        = ConfigManager:AllConfigs()
        local DefaultDropValue  = table.find(AllConfigs, ConfigName) and ConfigName or nil
        ...
        local BtnGroup = ConfigTab:Group({})
        ...
        BtnGroup:Button({
            ...
            Callback = function()
                if ConfigName == "" then
                    WindUI:Notify({ Title = "Config", Content = "Config name cannot be empty!", Duration = 3 })
                    return
                end
                Window.CurrentConfig = ConfigManager:Config(ConfigName)
                if Window.CurrentConfig:Save() then
                    WindUI:Notify({ Title = "Config Saved", Content = "'" .. ConfigName .. "' has been saved", Duration = 3 })
                end
        ...
        BtnGroup:Button({
            ...
            Callback = function()
                if ConfigName == "" then
                    WindUI:Notify({ Title = "Config", Content = "Enter or select a config name first!", Duration = 3 })
                    return
                end
                Window.CurrentConfig = ConfigManager:Config(ConfigName)
                if Window.CurrentConfig:Load() then
                    WindUI:Notify({ Title = "Config Loaded", Content = "'" .. ConfigName .. "' loaded successfully", Duration = 3 })
                else
                    WindUI:Notify({ Title = "Config", Content = "Config '" .. ConfigName .. "' not found", Duration = 3 })
                end
            end,
        })
        ...
        local BtnGroup2 = ConfigTab:Group({})
        ...
        BtnGroup2:Button({
            ...
            Callback = function()
                if ConfigName == "" then
                    WindUI:Notify({ Title = "Config", Content = "Select a config to delete first!", Duration = 3 })
                    return
                end
                pcall(function()
                    ConfigManager:Config(ConfigName):Delete()
                end)
                WindUI:Notify({ Title = "Config Deleted", Content = "'" .. ConfigName .. "' has been deleted", Duration = 3 })
                AllConfigsDropdown:Refresh(ConfigManager:AllConfigs())
            end,
        })
        ...
        BtnGroup2:Button({
            ...
            Callback = function()
                pcall(function()
                    ...
                    Window.CurrentConfig:Delete()
                end)
                -- Reset all flags to default (false)
                if _toggleAntiAFK        then _toggleAntiAFK:Set(false)        end
                if _toggleAutoReconnect  then _toggleAutoReconnect:Set(false)  end
                if _toggleAutoRejoinKick then _toggleAutoRejoinKick:Set(false) end
                WindUI:Notify({ Title = "Config", Content = "All settings restored to default", Duration = 3 })
                AllConfigsDropdown:Refresh(ConfigManager:AllConfigs())
            end,
        })
    end
